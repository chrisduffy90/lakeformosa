...
function isValidEmail(v2) {
  return typeof v2 === "string" && /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(v2.trim());
}
  ...
  async fetch(request, env) {
    ...
    try {
      ...
      if (pathname === "/get-involved" && request.method === "POST") {
        const { first_name, last_name = "", email, address = "", interests = [] } = await request.json();
      ...
      if (pathname === "/contact" && request.method === "POST") {
        const { name, email, subject = "General", message } = await request.json();
...
// .wrangler/tmp/bundle-0s0geT/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-0s0geT/middleware-loader.entry.ts
